...
module carA
	...
	[Move] xA = 1 & yA = floor(2*N/3) & dirA = east -> 
		p*10/8 : (xA' = min(xA + 1, N)) & (dirA' = east) +
		q*10/8 : (yA' = min(yA + 1, N)) & (dirA' = south) + 
		q*10/8 : (yA' = max(yA - 1, 1)) & (dirA' = north);

			
	[Move] xA = N-1 & yA = N & dirA = north -> 
		p*10/8 : (dirA' = north) & (yA' = max(yA - 1, 1)) +
		q*10/8 : (dirA' = west) & (xA' = max(xA - 1, 1)) + 
		q*10/8 : (dirA' = east) & (xA' = min(xA + 1, N));
	...
	[Move] xA != 1 & xA != N & yA = 1 & dirA = north -> 
			p*10/8 : (yA' = min(yA + 1, N)) & (dirA' = south) + 
			q*10/8 : (xA' = min(xA + 1, N)) & (dirA' = east) + 
			q*10/8 : (xA' = max(xA - 1, 1)) & (dirA' = west);
	
	[Move] xA != 1 & xA != N & yA = 1 & (dirA = east | dirA = west) -> 
			p*10/7 : (xA' = min(xA + 1, N)) & (dirA' = east) + 
			p*10/7 : (xA' = max(xA - 1, 1)) & (dirA' = west) + 
			q*10/7 : (yA' = min(yA + 1, N)) & (dirA' = south);


	
	//Part 6
	[Move] xA = N & yA != 1 & yA != N & dirA = east -> 
			p*10/8 : (dirA' = west) & (xA' = max(xA - 1, 1)) + 
			q*10/8 : (dirA' = north) & (yA' = max(yA - 1, 1)) + 
			q*10/8 : (dirA'  = south) & (yA' = min(yA + 1, N));

	[Move] xA = N & yA != 1 & yA != N & (dirA = north | dirA = south) -> 
			p*10/7 : (dirA' = north) & (yA' = max(yA - 1, 1)) + 
			p*10/7 : (dirA' = south) & (yA' = min(yA + 1, N)) + 
			q*10/7 : (dirA' = west) & (xA' = max(xA - 1, 1));
	...
	[Move] xA != 1 & xA != N & yA = N & (dirA = east | dirA = west) -> 
			p*10/7 : (dirA' = east) & (xA' = min(xA + 1, N)) + 
			p*10/7 : (dirA' = west) & (xA' = max(xA - 1, 1)) + 
			q*10/7 : (dirA' = north) & (yA' = max(yA - 1, 1)); 
	...
	[Move] xA = 1 & yA > 1 & yA < N & dirA = west -> 
			p*10/8 : (dirA' = east) & (xA' = min(xA + 1, N)) + 
			q*10/8 : (dirA' = north) & (yA' = max(yA - 1, 1)) + 
			q*10/8 : (dirA' = south) & (yA' = min(yA + 1, N));
	...
	[Move] xA = 1 & yA > 1 & yA < N & (dirA = north | dirA = south) -> 
			p*10/7 : (dirA' = north) & (yA' = max(yA - 1, 1)) + 
			p*10/7 : (dirA' = south) & (yA' = min(yA + 1, N)) + 
			q*10/7 : (dirA' = east) & (xA' = min(xA + 1, N));
